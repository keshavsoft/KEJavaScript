...
const CommonRegisterCommand = "AddCode.AddListeners";
...
const LocalFuncToActivate = () => {
    try {
        ...
        if (fse.existsSync(LocalOpenFilePath)) {
            ...
            StartFuncFromAlterEntryFile({ inFilePath: LocalOpenFilePath });
        } else {
            LocalFuncIfNotFound();
        };
...
const LocalFuncIfFound = () => {
    const LocalFromPath = path.join(__dirname, "copyCode", "AddListeners");
    const LocalToPath = LocalFuncGetActiveEditorPath();

    fse.copySync(LocalFromPath, path.join(LocalToPath, "AddListeners"));
};
